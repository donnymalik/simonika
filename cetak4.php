...
while($data=mysqli_fetch_array($query))
{
	...
	$pdf->Cell(0,1,$data['iv_niv'],0,9,'C');
	$pdf->ln(0.3);
	$pdf->SetFont('Times','',9);
	$pdf->SetY(4);
	$pdf->SetX(2);
	$pdf->MultiCell(15,0,"Di cetak pada       : ".date("D-d/m/Y"),0,'L');
	$pdf->SetFont('Times','',12);
	$pdf->SetY(6);
	$pdf->SetX(2);
	$pdf->Cell(4,0,"Nama Penggagas  ",0,0,'L');
	$pdf->Cell(1,0,": ",0,0,'C');
	$pdf->Cell(1.5,0,$data['iv_pgs'],0,1,'L');
	$pdf->SetY(6.5);
	$pdf->SetX(2);
	$pdf->Cell(4,0,"Nama Instansi  ",0,0,'L');
	$pdf->Cell(1,0,": ",0,0,'C');
	$pdf->Cell(1.5,0,$data['iv_nis'],0,1,'L');
	$pdf->SetY(7);
	$pdf->SetX(2);
	$pdf->Cell(4,0,"Jenis Inovasi  ",0,0,'L');
	$pdf->Cell(1,0,": ",0,0,'C');
	$pdf->Cell(1.5,0,$data['iv_ji'],0,1,'L');
	$pdf->SetY(7.5);
	$pdf->SetX(2);
	$pdf->Cell(4,0,"Angkatan  ",0,0,'L');
	$pdf->Cell(1,0,": ",0,0,'C');
	$pdf->Cell(1.5,0,$data['iv_akt'],0,1,'L');
	$pdf->SetY(8);
	$pdf->SetX(2);
	$pdf->Cell(4,0,"Tahun  ",0,0,'L');
	$pdf->Cell(1,0,": ",0,0,'C');
	$pdf->Cell(1.5,0,$data['iv_th'],0,1,'L');
	$pdf->SetY(8.5);
	$pdf->SetX(2);
	...
	$pdf->SetY(9);
	$pdf->SetX(2);
	$pdf->MultiCell(0,0.5,$data['iv_des'],0,'J');
